...
from io import BytesIO, \
    TextIOWrapper  # For the type of input of openStream and to create binary output streams for getting metadata.
...
class OpenPackagingConvention(FileInterface):
    ...
    xml_header = ET.ProcessingInstruction("xml",
    ...
    global_metadata_file = "/Metadata/opc_Global.json"  # Where the global metadata file is.
    opc_metadata_relationship_type = "http://schemas.ultimaker.org/package/2018/relationships/opc_metadata"  # Unique identifier of the relationship type that relates opc metadata to files.
    ...
    def setData(self, data: Dict[str, Any]) -> None:
        ...
        for virtual_path, value in data.items():
            if virtual_path.startswith(
                    self.metadata_prefix):  # Detect metadata by virtue of being in the Metadata folder.
    ...
    def getMetadata(self, virtual_path: str) -> Dict[str, Any]:
        ...
        if canonical_path.endswith("/size"):
            ...
            if self._resourceExists(requested_resource):
                result[self.metadata_prefix + virtual_path] = self.zipfile.getinfo(
    ...
    def getStream(self, virtual_path: str) -> BytesIO:
        ...
        if not self._resourceExists(
                virtual_path) and self.mode != OpenMode.WriteOnly:  # In write-only mode, create a new file instead of reading metadata.
    ...
    def addRelation(self, virtual_path: str, relation_type: str, origin: str = "") -> None:
        ...
        if origin not in self.relations:
            self.relations[origin] = ET.Element("Relationships",
    ...
    def _readRels(self) -> None:
        ...
        self.relations[""] = ET.Element("Relationships",
    ...
    def _readContentTypes(self) -> None:
        ...
        if not self.content_types_element:
            self.content_types_element = ET.Element("Types",
                                                    xmlns="http://schemas.openxmlformats.org/package/2006/content-types")
        # If there is no type for the .rels file, create it.
        if self.mode != OpenMode.ReadOnly:
            for type_element in self.content_types_element.iterfind(
                    "{http://schemas.openxmlformats.org/package/2006/content-types}Default"):
                if "Extension" in type_element.attrib and type_element.attrib["Extension"] == "rels":
                    break
            else:
                ET.SubElement(self.content_types_element, "Default", Extension="rels",
    ...
    def _writeContentTypes(self) -> None:
        ...
        self.zipfile.writestr(self.content_types_file,
                              ET.tostring(self.xml_header) + b"\n" + ET.tostring(self.content_types_element))
        ...
        for origin, relations_element in self.relations.items():
            for relationship in relations_element.iterfind(
                ...
                if relationship.attrib[
                    "Type"] != self.opc_metadata_relationship_type:  # Not interested in this one. It's not metadata that we recognise.
                    continue
    ...
    def _writeMetadata(self) -> None:
        ...
        keys_left = set(
            self.metadata.keys())  # The keys that are not associated with a particular file (global metadata).
