...
class UltimakerFormatPackage(FileInterface):
    ...
    _xml_header = ET.ProcessingInstruction("xml", "version=\"1.0\" encoding=\"UTF-8\"")  # Header element being put atop every XML file.
    _content_types_file = "/[Content_Types].xml"  # Where the content types file is.
    _global_metadata_file = "/Metadata/UFP_Global.json"  # Where the global metadata file is.
    _ufp_metadata_relationship_type = "http://schemas.ultimaker.org/package/2018/relationships/ufp_metadata"  # Unique identifier of the relationship type that relates UFP metadata to files.
    _metadata_prefix = "/metadata"
    _aliases = OrderedDict([  # Virtual path aliases. Keys are regex. Order matters!
    ...
    def listPaths(self) -> List[str]:
        ...
        return list(self._metadata.keys()) + [self._zipNameToVirtualPath(zip_name) for zip_name in self._zipfile.namelist()]
    ...
    def setData(self, data: Dict[str, Any]) -> None:
        ...
        for virtual_path, value in data.items():
            if virtual_path.startswith(self._metadata_prefix):  # Detect metadata by virtue of being in the Metadata folder.
    ...
    def getMetadata(self, virtual_path: str) -> Dict[str, Any]:
        ...
        if canonical_path.endswith("/size"):
            ...
            if self._resourceExists(requested_resource):
                result[self._metadata_prefix + virtual_path] = self._zipfile.getinfo(requested_resource.strip("/")).file_size
    ...
    def getStream(self, virtual_path: str) -> IO[bytes]:
        ...
        if not self._resourceExists(virtual_path) and self._mode != OpenMode.WriteOnly: # In write-only mode, create a new file instead of reading metadata.
    ...
    def addRelation(self, virtual_path: str, relation_type: str, origin: str="") -> None:
        ...
        if origin not in self._relations:
            self._relations[origin] = ET.Element("Relationships", xmlns="http://schemas.openxmlformats.org/package/2006/relationships")
        else:
    ...
    def _readRels(self) -> None:
        ...
        self._relations[""] = ET.Element("Relationships", xmlns="http://schemas.openxmlformats.org/package/2006/relationships") #There must always be a global relationships document.
    ...
    def _readContentTypes(self) -> None:
        ...
        if not self._content_types_element:
            self._content_types_element = ET.Element("Types", xmlns="http://schemas.openxmlformats.org/package/2006/content-types")
        # If there is no type for the .rels file, create it.
        if self._mode != OpenMode.ReadOnly:
            for type_element in self._content_types_element.iterfind("{http://schemas.openxmlformats.org/package/2006/content-types}Default"):
                if "Extension" in type_element.attrib and type_element.attrib["Extension"] == "rels":
                    break
            else:
                ET.SubElement(self._content_types_element, "Default", Extension="rels", ContentType="application/vnd.openxmlformats-package.relationships+xml")
    ...
    def _writeContentTypes(self) -> None:
        ...
        self._zipfile.writestr(self._content_types_file, ET.tostring(self._xml_header) + b"\n" + ET.tostring(self._content_types_element))
        ...
        for origin, relations_element in self._relations.items():
            for relationship in relations_element.iterfind("{http://schemas.openxmlformats.org/package/2006/relationships}Relationship"):
                if "Target" not in relationship.attrib or "Type" not in relationship.attrib: #These two are required, and we actually need them here. Better ignore this one.
                    continue
                if relationship.attrib["Type"] != self._ufp_metadata_relationship_type: #Not interested in this one. It's not metadata that we recognise.
                    continue
    ...
    def _writeMetadata(self) -> None:
        ...
        keys_left = set(self._metadata.keys()) #The keys that are not associated with a particular file (global metadata).
        ...
        if len(global_metadata) > 0:
            ...
            self.addRelation(self._global_metadata_file, self._ufp_metadata_relationship_type)
        for file_name, metadata in metadata_per_file.items():
            if len(metadata) > 0:
                self._writeMetadataToFile(metadata, file_name + ".json")
                self.addRelation(file_name + ".json", self._ufp_metadata_relationship_type)
